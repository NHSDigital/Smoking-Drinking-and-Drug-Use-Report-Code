...
def sdd_all(sdd_file: str) -> pd.DataFrame:
    ...
    data = import_sav_values(sdd_file, [])
    return data
...
def sdd_dtypes(sdd_metadata: str) -> Dict[str, str]:
    ...
    with open(param.TEACHER_META_DIR / "sdd_metadata.json", "r") as f:
        meta = json.load(f)
    dtype_map = {
        col: d["dtype"]
        for col, d in meta.items()
    }
    return dtype_map
...
def test_input_attributes(sdd_all: pd.DataFrame, sdd_dtypes: Dict[str, str]):
    ...
    exp_rows = param.NUM_TEACHER_ROWS  # Know exactly how many rows we should have
...
@pytest.mark.parametrize("column_index", list(range(0, 100)))
...
# Teacher file will have 0 continuous columns
@pytest.mark.slow
@pytest.mark.parametrize("column_index", list(range(0, 100)))
...
def test_unique_keys(sdd_all: pd.DataFrame):
    assert sdd_all["archschn"].is_unique, "ID column is not unique"
